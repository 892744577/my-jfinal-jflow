#namespace("bi.ranking")
  #sql ("contractRanKing")
      ...
      FROM
        aptenon_crm_contract as cct
      LEFT JOIN aptenon_admin_user as cau on cau.user_id = cct.owner_user_id
       left join aptenon_admin_dept as cad on cad.dept_id = cau.dept_id
      WHERE
        check_status = 1
  ...
  #sql ("receivablesRanKing")
      ...
      FROM
        aptenon_crm_receivables as cct
      LEFT JOIN aptenon_admin_user as cau on cau.user_id = cct.owner_user_id
      left join aptenon_admin_dept as cad on cad.dept_id = cau.dept_id
      WHERE
        check_status = 1
  ...
  #sql ("contractCountRanKing")
   ...
   #sql ("productCountRanKing")
     SELECT count(1) as `count`,cau.realname,cct.owner_user_id,cad.name as structureName
      FROM
        aptenon_crm_contract_product as ccp
      LEFT JOIN aptenon_crm_contract as cct on ccp.contract_id = cct.contract_id
      LEFT JOIN aptenon_admin_user as cau on cau.user_id = cct.owner_user_id
       left join aptenon_admin_dept as cad on cad.dept_id = cau.dept_id
      WHERE
        check_status = 1
  ...
  #sql ("customerCountRanKing")
      ...
      FROM
        aptenon_crm_customer as cct
      LEFT JOIN aptenon_admin_user as cau on cau.user_id = cct.create_user_id
       left join aptenon_admin_dept as cad on cad.dept_id = cau.dept_id
      WHERE
  ...
  #sql ("customerGenjinCountRanKing")
      ...
      FROM aptenon_admin_user as cau
      LEFT JOIN(select owner_user_id,customer_id from aptenon_crm_customer where
        #if(type == 1)
       ...
       left join aptenon_admin_dept as cad on cad.dept_id = cau.dept_id
       left join aptenon_admin_record as a on cct.customer_id = a.types_id and a.types = 'crm_customer'
      WHERE
         cau.user_id in (
         #for(i : userIds)
            #(for.index > 0 ? "," : "")#para(i)
  ...
  #sql ("recordCountRanKing")
      ...
      FROM
        aptenon_admin_record as ccr
      LEFT JOIN aptenon_admin_user as cau on cau.user_id = ccr.create_user_id
       left join aptenon_admin_dept as cad on cad.dept_id = cau.dept_id
      WHERE
   ...
   #sql ("contractProductRanKing")
     SELECT IFNULL(SUM(cccp.num),0) as num ,ccpc.`name` as categoryName,ccp.`name` as productName,ccpc.category_id , ccp.product_id
      FROM aptenon_crm_contract_product  as cccp
      LEFT JOIN aptenon_crm_contract as cct on cct.contract_id = cccp.contract_id
      LEFT JOIN aptenon_crm_product as ccp on ccp.product_id = cccp.product_id
      LEFT JOIN aptenon_crm_product_category as ccpc ON ccpc.category_id = ccp.category_id
      WHERE
        check_status = 1
        and  cct.owner_user_id in (
         #for(i : userIds)
  ...
  #sql ("travelCountRanKing")
      ...
      FROM
        aptenon_oa_examine_travel as coet
      LEFT JOIN aptenon_oa_examine as coe on coe.examine_id = coet.examine_id
      left join `aptenon_oa_examine_category` a on coe.category_id = a.category_id
      LEFT JOIN aptenon_admin_user as cau on cau.user_id = coe.create_user_id
       left join aptenon_admin_dept as cad on cad.dept_id = cau.dept_id
      WHERE a.type = 3 and
  ...
  #sql("productSellRanKing")
    SELECT cct.contract_id,ccs.customer_id,ccp.product_id,ccpc.category_id,cct.`name` as contractName,
      ...
      FROM aptenon_crm_contract as cct
      LEFT JOIN aptenon_crm_contract_product as cccp ON cccp.contract_id = cct.contract_id
      LEFT JOIN aptenon_crm_customer as ccs on ccs.customer_id = cct.customer_id
      LEFT JOIN aptenon_crm_product as ccp on ccp.product_id = cccp.product_id
      LEFT JOIN aptenon_crm_product_category as ccpc ON ccp.category_id = ccpc.category_id
      LEFT JOIN aptenon_admin_user as cau on cau.user_id = cct.owner_user_id
      where
          cct.check_status = 1
        and  cct.owner_user_id in (
      ...
      #sql("addressAnalyse")
        ...
        ifnull((SELECT COUNT(1) FROM aptenon_crm_customer where deal_status = 0 and left(address,INSTR(address, ',') - 1) = left(ccc.address,INSTR(ccc.address, ',') - 1)) ,0)as dealCustomer,
        #para(address) as address
        FROM aptenon_crm_customer as ccc
       ...
       #sql("portrait")
        ...
        FROM aptenon_crm_customer as a
        LEFT JOIN aptenon_admin_fieldv as b on a.batch_id=b.batch_id and b.`name`='客户行业'
       ...
       #sql("portraitLevel")
        ...
        FROM aptenon_crm_customer as a
        LEFT JOIN aptenon_admin_fieldv as b on a.batch_id=b.batch_id and b.`name`='客户级别'
        where   a.owner_user_id in (
         #for(i : userIds)
            #(for.index > 0 ? "," : "")#para(i)
         #end
       ...
       #sql("portraitSource")
        ...
        FROM aptenon_crm_customer as a
        LEFT JOIN aptenon_admin_fieldv as b on a.batch_id=b.batch_id and b.`name`='客户来源'
          where   a.owner_user_id in (
         #for(i : userIds)
            #(for.index > 0 ? "," : "")#para(i)
         #end
