#namespace("work.workbench")
  #sql("myTask")
    ...
    from aptenon_task a left join aptenon_work b on a.work_id = b.work_id where (a.owner_user_id like concat('%,',#para(userId),',%') or a.main_user_id = #para(userId)) and a.pid = 0  and a.ishidden = 0 and a.is_top = #para(isTop) and (a.status = 1 or a.status = 2) and a.is_archive = 0
  #end
  #sql("dateList")
