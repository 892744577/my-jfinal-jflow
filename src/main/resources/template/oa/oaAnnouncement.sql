#namespace("oa.announcement")
   #sql("queryById")
    ...
    from aptenon_oa_announcement as an
    LEFT JOIN aptenon_admin_user as us on us.user_id = create_user_id
    where
    an.announcement_id = ?
   #end
   #sql("queryList")
    ...
    FROM
      aptenon_oa_announcement AS a
    LEFT JOIN aptenon_admin_user as d on a.create_user_id=d.user_id
    LEFT JOIN aptenon_admin_user as b on FIND_IN_SET(b.user_id,a.owner_user_ids)
    LEFT JOIN aptenon_admin_dept as c on FIND_IN_SET(c.dept_id,a.dept_ids)
    WHERE 1=1
