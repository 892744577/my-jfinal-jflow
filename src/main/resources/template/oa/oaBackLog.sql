#namespace("oa.backLog")
  #sql ("queryEventNum")
  select count(*) from aptenon_oa_event
  where TO_DAYS(start_time) <= TO_DAYS(NOW()) and TO_DAYS(end_time) >= TO_DAYS(NOW())
  and (create_user_id = ? or owner_user_ids like CONCAT('%',?,'%'))
  #end

  #sql ("queryTaskNum")
  select count(*) from aptenon_task where status = 1 and (main_user_id = ? or owner_user_id like CONCAT('%,',?,',%'))
  #end

  #sql ("queryAnnouncementNum")
  select count(*) from aptenon_oa_announcement
  where TO_DAYS(start_time) <= TO_DAYS(NOW()) and TO_DAYS(end_time) >= TO_DAYS(NOW())
  and (read_user_ids not like CONCAT('%',?,'%') or read_user_ids is null)
  #end

  #sql ("queryLogNum")
  select count(*) from aptenon_oa_log
  where read_user_ids not like CONCAT('%,',?,',%') and (send_user_ids like CONCAT('%,',?,',%') or send_dept_ids like CONCAT('%,',?,',%'))
  #end

  #sql ("queryExamineNum")
  select count(*) from aptenon_oa_examine_record as a left join aptenon_oa_examine_log as b ON a.record_id = b.record_id
