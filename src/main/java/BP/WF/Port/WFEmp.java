...
public class WFEmp extends EntityNoName {
	...
	public final String getTelHtml() throws Exception {
		if (this.getTel().length() == 0) {
			...
			return "<a href=\"javascript:WinOpen('./Msg/SMS.aspx?Tel=" + this.getTel()
					+ "');\"  ><img src='/WF/Img/SMS.gif' border=0/>" + this.getTel() + "</a>";
		}
	}
	...
	public final String getEmailHtml() throws Exception {
		if (this.getEmail() == null || this.getEmail().length() == 0) {
			...
			return "<a href='Mailto:" + this.getEmail() + "' ><img src='/WF/Img/SMS.gif' border=0/>" + this.getEmail()
					+ "</a>";
		}
	}
	...
	public Map getEnMap() {
		...
		// map.AddTBStringDoc(WFEmpAttr.StartFlows, null, "可以发起的流程", true,
		// true);
	...
	protected boolean beforeUpdate() throws NumberFormatException, Exception {
		...
		// {
		// if (this.HisAlertWay == AlertWay.SMSAndEmail || this.HisAlertWay ==
		// AlertWay.Email)
