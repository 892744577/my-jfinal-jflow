        ...
        if ("contractNum".equals(type)){
            for (int i=1; i <= cycleNum;i++){
                sqlStringBuffer.append("select '").append(beginTime).append("' as month,count(contract_id) as thisMonth," +
                        "(select count(contract_id) from aptenon_crm_contract where DATE_FORMAT(order_date,'%Y%m ') = '")
                        .append(beginTime-1).append("' and check_status = 1 and owner_user_id in (").append(userIds)
                        .append(")) as lastMonth,(select count(contract_id) from aptenon_crm_contract where DATE_FORMAt(order_date,'%Y%m') = '")
                        .append(beginTime-100).append("' and check_status = 1  and owner_user_id in (").append(userIds)
                        .append(")) as lastYear from aptenon_crm_contract where DATE_FORMAT(order_date,'%Y%m') = '")
                        .append(beginTime).append("' and check_status = 1 and owner_user_id in (").append(userIds).append(")");
                if (i != cycleNum){
            ...
            for (int i=1; i <= cycleNum;i++){
                sqlStringBuffer.append("select '").append(beginTime).append("' as month,IFNULL(SUM(money),0) as thisMonth," +
                        "(select IFNULL(SUM(money),0) from aptenon_crm_contract where DATE_FORMAT(order_date,'%Y%m ') = '")
                        .append(beginTime-1).append("' and check_status = 1 and owner_user_id in (").append(userIds)
                        .append(")) as lastMonth,(select IFNULL(SUM(money),0) from aptenon_crm_contract where DATE_FORMAt(order_date,'%Y%m') = '")
                        .append(beginTime-100).append("' and check_status = 1  and owner_user_id in (").append(userIds)
                        .append(")) as lastYear from aptenon_crm_contract where DATE_FORMAT(order_date,'%Y%m') = '")
                        .append(beginTime).append("' and check_status = 1 and owner_user_id in (").append(userIds).append(")");
                if (i != cycleNum){
            ...
            for (int i=1; i <= cycleNum;i++){
                sqlStringBuffer.append("select '").append(beginTime).append("' as month,IFNULL(SUM(money),0) as thisMonth," +
                        "(select IFNULL(SUM(money),0) from aptenon_crm_receivables where DATE_FORMAT(return_time,'%Y%m ') = '")
                        .append(beginTime-1).append("' and check_status = 1 and owner_user_id in (").append(userIds)
                        .append(")) as lastMonth,(select IFNULL(SUM(money),0) from aptenon_crm_receivables where DATE_FORMAt(return_time,'%Y%m') = '")
                        .append(beginTime-100).append("' and check_status = 1  and owner_user_id in (").append(userIds)
                        .append(")) as lastYear from aptenon_crm_receivables where DATE_FORMAT(return_time,'%Y%m') = '")
                        .append(beginTime).append("' and check_status = 1 and owner_user_id in (").append(userIds).append(")");
                if (i != cycleNum){
                    sqlStringBuffer.append(" union all ");
                }
    ...
    public R totalContract(Integer deptId,Long userId,String type,String startTime,String endTime){
        ...
        for (int i=1; i <= cycleNum;i++){
            sqlStringBuffer.append("select '").append(beginTime).append("'as type,count(contract_id) as contractNum,IFNULL(SUM(IFNULL(money,0)),0) " +
                    "as contractMoney,IFNULL((select SUM(IFNULL(money,0)) from aptenon_crm_receivables as b where b.contract_id = a.contract_id),0)" +
                    " as receivablesMoney from aptenon_crm_contract as a where DATE_FORMAT(order_date,'").append(sqlDateFormat)
                    .append("') = '").append(beginTime).append("' and check_status = 1 and owner_user_id in (").append(userIds).append(")");
            if (i != cycleNum){
                sqlStringBuffer.append(" union all ");
            }
            beginTime = biTimeUtil.estimateTime(beginTime);
